...
class CartTest < ActiveSupport::TestCase
  ...
  test 'puede contener productos' do
    ...
    CartProduct.create(cart: cart, product: product, quantity: 1)

    assert cart.products.include?(product), 'El carrito debe contener el producto añadido'
  end
  ...
  test 'cantidad del producto en el carrito es correcta' do
    ...
    CartProduct.create(cart: cart, product: product, quantity: 3)
